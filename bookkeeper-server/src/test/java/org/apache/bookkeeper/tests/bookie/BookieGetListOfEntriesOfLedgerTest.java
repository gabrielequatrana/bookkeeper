...
public class BookieGetListOfEntriesOfLedgerTest {
	...
	public void getListOfEntriesOfLedgerTest() throws IOException, BookieException, InterruptedException {
		...
		// Add some entries to the bookie
		for (int i = 0; i <= numEntries; i++) {
			ByteBuf entry = TestUtil.generateEntry(ledgerId, (long) i);
			bookie.addEntry(entry, false, callback, null, new byte[0]);
		}
		
		// Get the iterator from the bookie
		...
		// Assert that the number of entries added to the bookie equals the length of the iterator
		assertEquals(numEntries, index);
	}
}
